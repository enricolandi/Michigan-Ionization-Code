...
;
; Fa l'interpolazione su height invece che sulla temperatura
...
pro wind_diagn_3,zeta,smooth=smooth,remote=remote,quiet=quiet,shift=shift,t_start=t_start,$
...
;  Nota: il files dei modelli devono avere le seguenti grandezze:
...
;
;  fileout       User-defined name of the file with the results to be saved
;
...
;
;     COMINCIA LA HISTORY DELLA VERSIONE SENZA IONIZZAZIONE MULTIPLA
;
;     V.16 - 27-Jan-2024 - Messa l'interpolazione delle ioniz e reco rate su 15 punti invece 
;                          che 3, che faceva casino quando i gradienti erano molto forti
...
; 1.2 - Si incazza se non gli dai l'elemento
   ...
   if not keyword_set(file_model) then begin
      ...
      if not keyword_set(remote) then begin
         ...
         if not keyword_set(no_plot) then $
            file_model=dialog_pickfile(filter='*.save',title='Scegli il modello di vento')
...
; 1.6 - Calculates electron density, assuming that the average mass is 0.61 m_p even if it is not fully ionized.
...
if not keyword_set(dens) then begin
   ...
   ratio=proton_dens(alog10(temp),/hydrogen,/quiet)
...
;d_limit=1.0d11 Valore originale
d_limit=1.0d12
...
if keyword_set(photoion) then begin
   ...
   openr,1,'~/cds/pro/photoion_params.txt'
   readf,1,stronzata
   ...
   restore,'~/giuda/see/daily_averages/timed_see_daily_irradiance_all.save'
...
;if max(diff) lt delta_ioneq_min and heo0 le 2.0 and equilibrium eq 0.0 and negativo gt 0. then begin  ; Evita un loop micidiale - EL cambio 12 Nov 2014
...
if max(diff) lt delta_min and max(diff) gt delta_ioneq_min or equilibrium eq 1 then begin
   ...
   if i eq 0 then begin
      ...
      ttt_used=ttt_used(1:*)     ; Leva il primo zero
      hhh_used=hhh_used(1:*)
      ddd_used=ddd_used(1:*)
      vvv_used=vvv_used(1:*)
...
;               strcompress(string(quale_neg,'(40i3)'))+'    '+$
   ...
   ; B - Setta i valori del prossimo step
